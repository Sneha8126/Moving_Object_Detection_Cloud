...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function switchCamera() {
    ...
    // Step 1: save the current device index/device for fallback purposes.
    ...
    // Step 2 & 3: stop the current stream completely and release the
    // hardware handle BEFORE requesting a new one. Android devices
    // (especially Samsung) generally cannot hold two active camera
    // streams at once, so this must happen first.
    if (mediaStream) {
      mediaStream.getTracks().forEach((track) => track.stop());
      mediaStream = null;
    }
    ...
    // Step 4: give the OS/browser a brief moment to actually free the
    // camera resource before we try to claim it again.
    await delay(300);

    try {
      // Step 5: open the next camera by its exact deviceId.
      const newStream = await openCameraByDeviceId(nextDevice.deviceId);

      currentDeviceIndex = nextIndex;
      syncCurrentDeviceIndex(newStream);

      const label = nextDevice.label || `Camera ${nextIndex + 1}`;
      console.log("Switched to camera:", label, nextDevice.deviceId);

      // Step 6: detection/recording/screenshots continue automatically —
      // captureAndSendFrame() simply keeps reading from webcamVideo, which
      // now points at the new stream.
      showNotification(`Switched to ${label}`, "success");

    } catch (err) {
      console.error("Camera switch failed:", err);
      showNotification("Camera switch failed. Reopening previous camera…", "error");

      // Step 7: automatically reopen the previous camera so the user
      // never loses the feed. Same stop-first + delay discipline applies,
      // though there's nothing left running at this point since we
      // already released the stream above.
      await delay(300);

      if (previousDevice) {
        try {
          const fallbackStream = await openCameraByDeviceId(previousDevice.deviceId);

          currentDeviceIndex = previousIndex;
          syncCurrentDeviceIndex(fallbackStream);

          const prevLabel = previousDevice.label || `Camera ${previousIndex + 1}`;
          console.log("Reopened previous camera:", prevLabel, previousDevice.deviceId);
          showNotification(`Camera switch failed — restored ${prevLabel}.`, "error");

        } catch (fallbackErr) {
          // Absolute last resort: previous camera also failed to reopen.
          // The user genuinely has no active feed at this point, so we
          // shut the pipeline down cleanly instead of leaving it in a
          // broken/inconsistent state.
          console.error("Failed to reopen previous camera:", fallbackErr);
          showNotification("Camera unavailable: " + fallbackErr.message, "error");
        ...
        console.error("No previous device to fall back to.");
